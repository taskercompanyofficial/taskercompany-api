...
public function fetchComplaints(Request $request)
{
    ...
    // Start the query to fetch complaints
    $complaintsQuery = Complaint::query();

    // Check if the user is a branch manager, and filter complaints by their branch_id
    if ($user->role === 'branch-manager') {
        $complaintsQuery->where('branch_id', $user->branch_id);
    } else {
        // Filter complaints by technician for non-branch-manager users
        $complaintsQuery->where('technician', $user->id);
    }

    // Apply the status filter (if provided)
    if ($status) {
        if ($status === 'closed') {
            $complaintsQuery->where('status', 'closed');
        } elseif ($status === 'open') {
            $complaintsQuery->where('status', '!=', 'closed');
        }
    }

    // Fetch the filtered complaints
    $complaints = $complaintsQuery->get();

    // Get the total count of complaints (all statuses for the relevant technician/branch)
    $totalComplaintsCount = Complaint::where(function($query) use ($user) {
        if ($user->role === 'branch-manager') {
            $query->where('branch_id', $user->branch_id);
        } else {
            $query->where('technician', $user->id);
        }
    })->count();

    // Get the count of closed complaints
    $closedComplaintsCount = Complaint::where(function($query) use ($user) {
        if ($user->role === 'branch-manager') {
            $query->where('branch_id', $user->branch_id);
        } else {
            $query->where('technician', $user->id);
    ...
    // Calculate the in-progress count (total - closed)
    $inProgressComplaintsCount = $totalComplaintsCount - $closedComplaintsCount;

    // Build the complaints_count_by_status object
    $complaintsCountByStatusFormatted = [
        'open' => $inProgressComplaintsCount,
        'closed' => $closedComplaintsCount,
        'total' => $totalComplaintsCount,
    ];

    // Return the response with complaints and count data
    return response()->json([
        'data' => $complaints, // Filtered complaints based on status and role
        'complaints_count_by_status' => $complaintsCountByStatusFormatted
    ]);
}
